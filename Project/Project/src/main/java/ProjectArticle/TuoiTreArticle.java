...
public class TuoiTreArticle extends Application {
        ...
        try {
            ...
            if (category.equals("Newest")) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    // Add date&time
                    String date = Helper.convertTime1(getTTArticleDate("http://tuoitre.vn" + titleAndLink.get(i).attr("href")));
                    tuoitreArticleList.get(i).setDate(Helper.formatTime(date).substring(0, 16));
                    // Add time duration
                    tuoitreArticleList.get(i).setTimeDuration(Helper.getConvertedTimeDuration(date));
                    // Add thumbnail
    ...
    public static ArrayList<Article> getListOfSearchTTArticle(String keyword, String category) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // Add date&time
                String date = Helper.convertTime1(getTTArticleDate("http://tuoitre.vn" + titleAndLink.get(i).attr("href")));
                listOfSearchArticle.get(i).setDate(Helper.formatTime(date).substring(0, 16));
                // Add time duration
    ...
    public static void displayTTArticle(Article a, VBox vbox) throws IOException {
